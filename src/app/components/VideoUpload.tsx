...
export function VideoUpload({ onGradingComplete, isProcessing, setIsProcessing }: VideoUploadProps) {
  ...
  const parseStructuredResponse = (raw: any): Partial<GradingResult> => {
    ...
    if (raw && typeof raw === "object" && !Array.isArray(raw) && (raw as any).algorithmSteps) {
      return raw as Partial<GradingResult>;
    }

    // Step 2: unwrap common n8n envelope shapes
    // handles: { output: "..." }, [ { output: "..." } ], { text: "..." }, { result: "..." }
    ...
    // Step 3: unwrapped object may already be correct
    if (candidate && typeof candidate === "object" && !Array.isArray(candidate) && (candidate as any).algorithmSteps) {
      return candidate as Partial<GradingResult>;
    }

    // Step 4: work with a string from here
    const text: string = typeof candidate === "string" ? candidate : JSON.stringify(candidate ?? raw);

    // Step 5: strip code fences  ```json ... ```  or  ``` ... ```
    const fenceMatch = text.match(/```(?:json)?\s*([\s\S]*?)```/i);
    const unwrapped = fenceMatch ? fenceMatch[1].trim() : text.trim();

    // Step 6: direct JSON parse on unwrapped text
    try {
      const parsed = JSON.parse(unwrapped);
      if (parsed && typeof parsed === "object") return parsed as Partial<GradingResult>;
    } catch (_) {}

    // Step 7: pluck first {...} block
    ...
    // Step 8: last resort markdown parser
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Always pass the raw response directly into parseStructuredResponse —
      // it handles all n8n envelope shapes (JSON object, array, plain text, code fences).
